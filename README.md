...
| `tests/git_parity/test_behavior_parity.py` | 430+ byte-for-byte behavior cases (return code, stdout, stderr) run against the real 2.54.0 oracle in a hermetic, deterministic environment — covers rev-parse, rev-list, config, status, diff/diff-tree/diff-index/diff-files (incl. `-R`, `--diff-filter`, `-U<n>`, hunk section headings), log/show (all `--pretty`/`--format` placeholders, every `--date=<mode>`, `--graph`, `--topo-order`, `--no-walk`, multi-rev `show`, `--grep`/`--author`/`-S`/`-G` filters), describe, name-rev, ls-tree/ls-files, for-each-ref (`%(...)` format atoms), grep (`-w`/`-v`/`-F`/tree-grep), blame (`-L`/`-l`), archive (tar, `--prefix`), commit, merge, merge-tree, branch (`--merged`), tag, reflog, worktree, checkout, and more |
