...
| `tests/git_parity/test_behavior_parity.py` | 2410+ byte-for-byte behavior cases (return code, stdout, stderr) run against the real 2.54.0 oracle in a hermetic, deterministic environment — covers rev-parse (incl. `<ref>@{<n>}` reflog selectors with `^`/`~` suffixes), rev-list (commit-date/`--topo-order`/`--date-order` traversal, `--branches`/`--tags`/`--remotes` namespaces, `--header`, `--abbrev-commit`/`--abbrev`, `-z`, `--disk-usage`, `--objects`/`--objects-edge` boundary exclusion, `--bisect`/`--bisect-all`, `--children`, `--parents`), config, status (incl. rename detection on by default with `--no-renames`/`--renames` and `-M`/`--find-renames[=<n>]` thresholds — `-M` force-enables detection even after `--no-renames`, matching `builtin/commit.c`; and `-v`/`-vv` which append the staged diff, then for `-vv` the worktree diff under `c/i` and `i/w` prefixes with a 50-dash separator, placed before the summary line per `wt_longstatus_print`), diff/diff-tree/diff-index/diff-files (incl. `-R`, `--diff-filter`, `-U<n>`, diff-files `-p`/`--name-status`/`-z`/`--abbrev`/`--full-index`/`--patch-with-raw` with full-id raw default, hunk section headings, `--check` whitespace errors, `--word-diff`/`=porcelain` inline markers), log/show (all `--pretty`/`--format` placeholders incl. reflog `%gd`/`%gD`/`%gs`/`%gn`/`%ge`, the `%aN`/`%aE`/`%cN`/`%cE` mailmap-resolved identity placeholders and `--use-mailmap`/`--no-use-mailmap` (builtin formats honor `.mailmap` by default per `log.mailmap`, while `%an`/`%ae` stay raw — all four `.mailmap` line forms via a `mailmap.c` port shared with `check-mailmap`), every `--date=<mode>`, `--graph`, `--topo-order`, `--no-walk`, `-g`/`--walk-reflogs` with `--date` selectors, `--follow` across renames, `--oneline`/`--format` with `--stat`/`-p`/`--shortstat`/`--name-only`, pathspec-restricted diffs (`log -p/--stat/--raw <path>`), multi-rev `show`, `--grep`/`--author`/`-S`/`-G` filters), describe (incl. `--contains`), name-rev (incl. annotated-tag `^0` dereference), ls-tree, ls-files (`-o`/`-i` with `-x`/`-X`/`--exclude-standard` exclusion, `--format` atoms + conflict error, `-v`/`-f` tags, `-u` unmerged stages, grouped combined selectors), for-each-ref (`%(...)` format atoms incl. `refname:lstrip`/`rstrip` and `%(if)`/`%(then)`/`%(else)`/`%(end)` conditionals; `--shell`/`--perl`/`--python`/`--tcl` quoting, `--points-at`, `--merged`/`--no-merged`, `--contains`/`--no-contains`, `--exclude`, `--start-after`, `--stdin`, `--include-root-refs`), rev-parse `--symbolic`, branch (`--merged`/`--no-merged`, `--contains`/`--no-contains`, `--points-at`, `-i`, `--format` ref-filter atoms, `-f` force/already-exists guard, `-t`/`--track` and `-u`/`--set-upstream-to` tracking config), grep (`-w`/`-v`/`-F`/tree-grep), blame (`-L`/`-l`/`-p`/`--line-porcelain`), format-patch (`--no-stat`/`--signoff`/`-v<n>`/`<rev>..` range), describe (`--all`), restore (`--source`/`--staged`/`--worktree`), reset (`--soft`/`--mixed`/`--hard`/`--merge`/`--keep` with the two-way-merge abort messages, the full-tree "Unstaged changes after reset:" report, `--no-refresh`/`-q`, `--pathspec-from-file`, `-N`/`--intent-to-add`), range-diff (commit matching with `=`/`!`/`<`/`>` and the inter-diff body), archive (tar, `--prefix`), ls-remote (local path: HEAD/refs/peeled tags, `--refs`, `--symref`, `-b`/`--branches`/`--tags`, `--get-url`, `--sort`, patterns, `--exit-code`), apply (`--check`/`--stat`/`--numstat`, `--summary`, `--exclude`/`--include`, `-v`/`-q`, new-file creation), cat-file (`--batch`/`--batch-check`/`--batch-command`/`--batch-all-objects`, `--batch-check=<format>` atoms), diff-tree (`--no-commit-id`), count-objects (`-v`), show-ref (`--head`, `--branches`/`--heads`/`--tags`, `-d` annotated-tag `^{}` peeling, `-s`/`--hash[=<n>]` and `--abbrev[=<n>]` with `[4,40]`/`0`-full clamping, `-q`, tail-component pattern match, `--verify` with safe pseudo-refs, `--exists` literal rc 0/2 check, `--exclude-existing` stdin filter), mktree (ls-tree-format stdin → tree, `%o` mode verbatim, type/availability checks, `--missing`/`--batch`/`-z`, empty input → empty tree), shortlog (`-c` committer grouping, `-e`, `-n`, `-s`, `--format`/`--pretty` per-commit records incl. the `reference` style and the `invalid --pretty format` error, `-w[<width>,<i1>,<i2>]` line-wrapping ported from `strbuf_add_wrapped_text`), pack-refs (default packs `refs/tags/*` and prunes loose copies, `--all`, `--no-prune`, `--include`/`--exclude` globs that replace/trim the default set, `--auto`'s size-scaled `log2u` threshold, fully-peeled `^` tag lines, exact header — verified by direct `packed-refs`/loose-ref state comparison), update-index (`--assume-unchanged`/`--no-assume-unchanged` with `ls-files -v`/status effects, `--index-info`, `--stdin`/`-z`, `-q`, `--really-refresh`), commit (`--dry-run` plus the `--short`/`--porcelain`/`--long`/`-z`/`--branch` dry-run output formats that all imply `--dry-run`, `-C`/`-c` message+author reuse with C Git's `Date:`-line rule, `--squash`/`--fixup` incl. `amend:`/`reword:` prefixes and the same-target subject-strip, `-m`/`-F` body composition under the autosquash header, the `-m`/`-C`/`-c`/`-F`/`--squash`/`--fixup` incompatibility errors with exact name ordering, `--pathspec-from-file`/`--pathspec-file-nul`, the `--reset-author` requires-`-C`/`-c`/`--amend` guard), merge (incl. `-q`, `-n`/`--stat`, `-s ort`/`recursive`/`ours` + unknown-strategy error, `-X ours`/`theirs`, `-F`/`--file`, `--no-verify`, the `Auto-merging <path>` notice), merge-tree, tag (`--merged`/`--no-merged`, `--column`/`--no-column`, `-i`, `-v`/`--verify`, `-F`/`--trailer`/multi-`-m`, `--create-reflog`), add (`-u`/`--update`, `--no-all`/`--ignore-removal`, `--pathspec-from-file`, `-N`/`--intent-to-add`), notes (`add`/`append` with `-m`/`-F`/`-C`/`--separator`/`-f`/`--allow-empty`, `copy`, `remove`, `list [<obj>]`, `prune`, `get-ref`, `--ref`), remote (`add` with `-t`/`-m`/`--tags`/`--no-tags`/`--mirror`, `rename`, `remove`, `set-url`/`get-url` with `--push`/`--add`/`--delete`/`--all`, `set-branches --add`, `set-head -d`), worktree, checkout, show-branch (legend + `+`/`*`/`!`/`-` marker matrix + `^`/`~` commit naming, `--merge-base`/`--independent`/`--reflog`, `-a`, `--topo-order`/`--date-order`, `--current`, `--list`, `--no-name`/`--sha1-name`, `--more=<n>`, `--topics`, `--color=always` per-column palette), stash (byte-identical commits, `show`/`show -p`, `stash@{N}` selectors, list ordering, `push`/`save` with `-m`/`-q`/`-k`/`--keep-index`, bare-flag default to `push`), reflog (incl. `--date=<mode>` selectors), clean (`-n`/`--dry-run`, `-q`, `-e`/`--exclude=<pat>` with `!`-negation, `-x` removing ignored files, `-X` only-ignored, `-d` directory recursion incl. the `-dX` ignored-dir collapse, the `-x`/`-X` mutual-exclusion error and the no-`-f` `clean.requireForce` guard), replace (`-l` listing with `--format=short`/`medium`/`long`, ref creation with the already-exists guard and `-f` overwrite, the object type-mismatch error, `-d` deletion, `--graft` parent rewriting incl. the unnecessary-graft notice, `--convert-graft-file`, and the `-d`/`-l`/`-f` command-mode conflicts), switch (`-c`/`-C` create/force-create, `-d`/`--detach` of branches and lightweight/annotated tags, `-q`, the two-refs/no-arg/invalid-ref/non-branch-needs-detach/`-c`-already-exists errors, and the `Previous HEAD position`/`Switched to` messages), interpret-trailers (a `trailer.c` port: `--trailer`, `--parse`, `--only-trailers`, `--only-input`, `--trim-empty`, `--in-place`, `--if-exists`/`--where` placement, `---` divider detection and `--no-divider`, the empty-token error, and `trailer.where`/`trailer.<token>.key` alias config), init (`--shared=group`/`all`/`<octal>`/`umask`/bool permission masks with bad-value errors, `--separate-git-dir` incl. the `--bare` conflict, `--template` custom/missing-dir), archive `-v` verbose listing and `--mtime=@<epoch>`/ISO-with-tz timestamps, update-index `--again` (re-stage already-tracked paths, pathspec-scoped, missing-file abort), log/show `-q`/`--quiet` (suppresses the diff except when `-p`/`--stat`/`--raw` re-enable it, with the `--name-only` conflict) and `--clear-decorations`, pack-redundant's `--i-still-use-this` deprecation gate, bugreport (`-s`/`--suffix` strftime filename suffix incl. `--no-suffix` and last-wins ordering, `-o`/`--output-directory` with leading-dir creation, the parse-options 'requires a value' rc-129 errors, `--diagnose=<bad>` value validation, the two usage variants — short `usage()` for a stray arg vs full `usage_with_options()` for `-h`/unknown options — and the `O_EXCL` duplicate-file fatal), diagnose (`-s`/`--suffix`, `--mode=stats`/`all` with the invalid-mode rc-129 error, `-o`), init/init-db `--ref-format=files` (with the `unknown ref storage format` fatal for bad values; `reftable` honestly rejected), fast-export `--no-data` (omit blob records, reference blobs by full sha) and `--data` with last-wins, update-index `--unresolve`/`--clear-resolve-undo` (resolve-undo restore after a merge conflict) and `--fsmonitor-valid`/`--no-fsmonitor-valid`, backfill `--min-batch-size` (no-op in a complete repo), archive's `--exec`-requires-`--remote` fatal, cat-file `--follow-symlinks` (in-tree symlink resolution for `<rev>:<path>` under `--batch`/`--batch-check`, incl. dangling/out-of-tree/loop/not-a-directory special outputs), and the `branch -m`/`-M` rename ref+reflog state incl. the same-name no-op move that must preserve `refs/heads/<name>` (a previously-destructive case), apply (the full engine: `-p<n>` strip, `--no-add`, `-N`/`--intent-to-add`, `-z`, `-C<n>` context fuzz, `-R`/`--reverse`, `--include`/`--exclude`, `--unidiff-zero`, `--allow-overlap`, and a real `--3way` reconstructing the preimage blob and doing an in-core 3-way merge with conflict markers + index stages 1/2/3, incl. `--ours`/`--theirs`/`--union` resolution), diff-pairs (the full plumbing: `-z` raw-pair stdin wired to the shared diff engine honoring every `--patch`/`--raw`/`--stat`/`--numstat`/`-M`/`-C`/`-B`/`-S`/`-G`/whitespace/`--diff-algorithm`/prefix/filter option), init/init-db `--ref-format=reftable` (a byte-exact reftable on-disk backend — prefix-compressed ref/log/obj blocks, restart points, CRC32 footer, geometric stack compaction — that real git reads back and `fsck`s clean), hash-object external `filter.<drv>.clean`/`.process` clean filters and `working-tree-encoding` re-encoding, fast-export `--anonymize`/`--anonymize-map` (deterministic token anonymization), archive `--remote`/`--exec` over the local upload-archive protocol (incl. byte-exact `tgz`/`tar.gz`), bugreport `--diagnose` and diagnose (the report/diagnostics-zip structure, `--mode`, suffix), the interactive patch machinery shared by add/stage/commit/reset/stash (`-p`/`--patch`, `-i`/`--interactive`, `-e`/`--edit`, `-U<n>`/`--inter-hunk-context` — a port of `add-patch.c`/`add-interactive.c`: the `y/n/q/a/d/s/e/k/K/j/J/g/?` hunk-selection loop with mode-specific prompts (Stage/Unstage/Discard/Apply/Stash), hunk splitting and manual edit, driven via scripted stdin), diff-files `-c`/`--cc` combined diff over unmerged stages (a `combine-diff.c` port), difftool (`-x`/`--extcmd`, `-d`/`--dir-diff`, `-y`/`--no-prompt`, `-g`/`--gui`, `--no-index`, the `Launch '<tool>'` prompt), rev-list `--sparse` traversal, fetch/pull/push over local remotes (a `localtransport` port: `push` new-branch/`-f`/`-d`/`-u`/`--mirror`/`--all`/`--tags`/non-ff-rejection/`--force-with-lease`, the `To <path>` + ` * [new branch]`/`<old>..<new>` summary lines; `fetch` `--all`/`--prune`/`--no-tags`/`--dry-run`/tag-following/`FETCH_HEAD`; `pull` fast-forward/`--ff-only`/`--rebase`/`--no-rebase` real-merge with the byte-exact merge commit), `apply --add`, `reset -U<n>`/`--unified`/`--inter-hunk-context` (the `requires '--patch'` guard and patch-context wiring), the `stash` unknown-option usage block, GPG signing + verification (a `gpg-interface.c` port shelling out to real gpg: `commit -S`/`--gpg-sign`/`--no-gpg-sign` with `commit.gpgsign`, `tag -s`/`-u`/`--local-user`, `verify-commit`, `verify-tag` — cross-verified by having the oracle accept pygit-signed objects — plus the sign-failure rc128 paths and the `%G?`/`%GS`/`%GK` log placeholders), `commit -t`/`--template` (template seeding with the missing-file fatal and editor flow), am (a full mailbox-apply engine: `mailsplit.c`/`mailinfo.c` ports — mbox/maildir splitting, RFC2822 folding, RFC2047 B/Q + base64/quoted-printable decode, multipart MIME, `[PATCH]` subject cleanup, scissors, quoted-cr — driving the apply engine with author/date preservation, the `.git/rebase-apply` state machine for `--continue`/`--skip`/`--abort`/`--quit`/`--show-current-patch`, `-3`/`--3way` fallback with conflict markers, `--empty=stop/drop/keep`, `--signoff`, `--keep-cr`, `--whitespace`, `-i` interactive, and the applypatch hooks; plus standalone `mailsplit`/`mailinfo`), repack (`-a`/`-A`/`-d`/`-f`/`-F`/`-l`/`-n`/`-q`/`--window`/`--depth`/`--max-pack-size`/`--cruft`/`--write-midx`/`-m`/geometric `-g`, verified via `count-objects -v`/`fsck`/object-set reachability since pack bytes are non-deterministic), fast-import (a stream-language interpreter: blob/commit/tag/reset/checkpoint, marks, `data`, `M`/`D`/`C`/`R`/`deleteall`, `from`/`merge`, the `--date-format`/`--export-marks`/`--max-pack-size`/`--big-file-threshold`/`--active-branches`/`--depth` flags), count-objects `-v` `prune-packable`, multi-pack-index silent verify, mailsplit `-o<dir>` attached form, rev-list `--sparse` traversal, stash (`push`/`save` with `-u`/`--include-untracked`, `-a`/`--all`, `-S`/`--staged`, real partial-pathspec stashing via `--pathspec-from-file`/`--pathspec-file-nul`, the `No local changes to save` and `Saved working directory and index state` messages and the `refs/stash` commit graph; `show -u`/`--only-untracked`; `export --print`/`--to-ref`), hash-object (`--filters`/`--no-filters`/`--path` gitattributes-driven clean conversion — a `convert.c` port covering `text`/`text=auto`/`eol=lf`/`eol=crlf`/`-text` CRLF normalization with git's binary-detection heuristics, `core.autocrlf`/`core.eol`, `core.safecrlf` warn/die, and the `ident` `$Id$` clean — plus the `--stdin`/`--stdin-paths` and argument-combination/usage errors), diff-files (the shared diff engine extended with `-M`/`--find-renames`, `-C`/`--find-copies`/`--find-copies-harder`, `-B`/`--break-rewrites`, `-S`/`-G` pickaxe with `--pickaxe-all`, `-O<orderfile>`, `-l<n>`), log/show `-L` line-level history (a `line-log.c` port: `<start>,<end>:<file>` and `:<funcname>:<file>` range tracking with the scoped `@@` hunks across history), and more |
