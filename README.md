...
| `tests/git_parity/test_behavior_parity.py` | 500+ byte-for-byte behavior cases (return code, stdout, stderr) run against the real 2.54.0 oracle in a hermetic, deterministic environment — covers rev-parse (incl. `<ref>@{<n>}` reflog selectors with `^`/`~` suffixes), rev-list, config, status, diff/diff-tree/diff-index/diff-files (incl. `-R`, `--diff-filter`, `-U<n>`, hunk section headings), log/show (all `--pretty`/`--format` placeholders incl. reflog `%gd`/`%gD`/`%gs`/`%gn`/`%ge`, every `--date=<mode>`, `--graph`, `--topo-order`, `--no-walk`, `-g`/`--walk-reflogs` with `--date` selectors, multi-rev `show`, `--grep`/`--author`/`-S`/`-G` filters), describe (incl. `--contains`), name-rev (incl. annotated-tag `^0` dereference), ls-tree/ls-files, for-each-ref (`%(...)` format atoms), branch (`--merged`, `--format` ref-filter atoms), grep (`-w`/`-v`/`-F`/tree-grep), blame (`-L`/`-l`), archive (tar, `--prefix`), cat-file (`--batch`/`--batch-check`/`--batch-command`/`--batch-all-objects`), count-objects (`-v`), commit, merge (incl. `-q`), merge-tree, tag, notes, worktree, checkout, show-branch (legend + `+`/`*`/`!`/`-` marker matrix + `^`/`~` commit naming, `--merge-base`/`--independent`/`--reflog`), stash (byte-identical commits, `show`/`show -p`, `stash@{N}` selectors, list ordering), reflog (incl. `--date=<mode>` selectors), and exact reflog messages for commit/`--amend`/reset/checkout/`branch -m`/merge/cherry-pick/revert (incl. zero-oid deletion-marker numbering), and more |
